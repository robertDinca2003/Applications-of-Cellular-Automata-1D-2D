...
void Custom2DMenu::DisplayContent() const {
    if (this->state == 0) {
        std::cout << "Custom 2D Automaton Visualization\n";
        std::cout << "1. New Random Convolution Matrix\n";
        std::cout << "2. Change function\n";
        std::cout << "3. Generate\n";
        std::cout << "4. Back\n";
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++)
                std::cout << this->game->getKernelElem(i, j) << ' ';
            std::cout << '\n';
        }
...
void Custom2DMenu::DisplayScreen(sf::RenderWindow *window) {
    ...
    std::vector<std::string> content(8);
    content[0] = "Custom 2D Automaton Visualization\n";
    content[1] = "1. New Random Convolution Matrix\n";
    content[2] = "2. Change Function " + std::to_string(this->function);
    ...
    if (this->state == 0)
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++) {
                ...
                window->draw(square);
                text.setString(std::to_string(static_cast<int>(this->game->getKernelElem(i, j) * 100) / 100.f));
